...
mod spi;
use crate::spi::{SpiState, MAX_ADC_PINS_I8, REGISTERSHANDLE};
...
#[cfg(debug_assertions)]
fn send_tuple(
    state: (PowerStateMachine, PowerStateMachine),
...
fn process_fsm(mut hdwr: Hardware) -> Hardware {
    ...
    let newstate = match hdwr.machine_state.0 {
        ...
        PowerStateMachine::MCUOff => change_state(PowerStateMachine::PowerDown, hdwr.machine_state),
        ...
        PowerStateMachine::ButtonRelease => {
            match interrupt::free(|cs| {
                ...
                ButtonRelease::UpLong => match hdwr.machine_state.1 {
                    PowerStateMachine::Wait => {
                ...
                ButtonRelease::UpShort => match hdwr.machine_state.1 {
                    PowerStateMachine::Wait => {
                        change_state(PowerStateMachine::WaitEntry, hdwr.machine_state)
                    }
                    PowerStateMachine::SignaledOn => {
                ...
                ButtonRelease::Down => hdwr.machine_state,
            }
        }
    };
    ...
    // handle adc readings
    let channels = interrupt::free(|_cs| unsafe { REGISTERSHANDLE.get_adc_channels_selected() });
    if channels != 0 {
        ...
        let adc_result: core::result::Result<u16, _> = match hdwr.current_channel {
            ...
            6 => hdwr
                .adc
                .borrow_mut()
                .read(&mut chart_plotter_hat::adc::channel::ADC6),
            #[cfg(not(feature = "proto-board"))]
            7 => hdwr
                .adc
                .borrow_mut()
                .read(&mut chart_plotter_hat::adc::channel::ADC7),
            ...
            Ok(adc_reading) => {
                ...
                interrupt::free(|cs| unsafe {
                    REGISTERSHANDLE.set_value(
                        hdwr.current_channel.try_into().unwrap(),
                        adc_reading,
                        cs,
                    );
                });
